...
//信号处理
func (p *Pdo) sysSignalHandle() {
	c := make(chan os.Signal, 1)
	signal.Notify(c, os.Interrupt)
	go func() {
		for sig := range c {
			log.Warnf("Ctrl+c,recode fail list to "+p.FailFile+" ,signal:%s", sig)
			for x := p.JobFinished - 1; x < p.JobTotal; x++ {
				CreateAppendFile(p.FailFile, p.JobList[x].Host+" "+p.JobList[x].Path)
			}
			os.Exit(0)
		}
	}()
...
//去重Host+Path
func (p *Pdo)UniqHosts(lists []HostList, list HostList) bool {
